	...
	for ( i = 0; i < MAX_EXPLOSION_NUM; i++ ) {
		if ( exExplode[i].bActive ) {
			...
			if ( exExplode[i].fRadius > EXPLOSION_R2 ) {
				...
				//ここに色々と書く
				exExplode[i].matMatrix.r[3].m128_f32[0] = exExplode[i].v3CenterPos.x;
				exExplode[i].matMatrix.r[3].m128_f32[2] = exExplode[i].v3CenterPos.z;
				exExplode[i].matMatrix.r[0].m128_f32[0] = exExplode[i].fRadius;
				exExplode[i].matMatrix.r[1].m128_f32[1] = exExplode[i].fRadius;
				exExplode[i].matMatrix.r[2].m128_f32[2] = exExplode[i].fRadius;
